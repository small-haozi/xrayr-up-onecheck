...
```
# 携带参数执行脚本
haha 节点id 节点类型 对接域名 对接密钥 上报阈值 是否开启审计 是否优化连接配置 解锁类型 "解锁项目,以逗号隔开"
```
...
```
curl -s https://raw.githubusercontent.com/small-haozi/xrayr-up-onecheck/main/install.sh | bash curl -sSL https://raw.githubusercontent.com/yourusername/yourrepo/master/install.sh | bash -s -- <解锁项目的uuid> 节点id 节点类型 对接域名 对接密钥 上报阈值 是否开启审计 是否优化连接配置 解锁类型 "解锁项目,以逗号隔开"
```
